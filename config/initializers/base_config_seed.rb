Rails.application.config.after_initialize do
  class BaseConfigSeed
    def import
      begin
        base_config_yml.fetch('Category', {}).each do |category_name, category_hash|
          category_obj = Category.find_or_initialize_by(slug: category_hash['slug'])
          category_obj.name =category_hash['name']
          category_obj.is_active = category_hash['is_active'] if !category_hash['is_active'].nil?
          save_object(category_obj)
            #---------Subcategories -----------------------------
          category_hash['sub_categories'].each do |sub_category_name, sub_category_hash|
            scg_obj = category_obj.sub_categories.find_or_initialize_by(slug: sub_category_hash['slug'])
            scg_obj.name =sub_category_hash['name']
            scg_obj.is_active = sub_category_hash['is_active'] if !sub_category_hash['is_active'].nil?
            save_object(scg_obj)
          end
        end
      rescue => e
